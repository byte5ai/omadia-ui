...
const rendererHtml = (name: string): string =>
...
export default defineConfig({
  ...
  renderer: {
    ...
    // Two HTML entries: the app (index) and the startup splash. The splash is a
    // standalone page (no React/fonts) so it paints instantly while index boots.
    build: {
      rollupOptions: {
        input: {
          index: rendererHtml('index.html'),
          splash: rendererHtml('splash.html'),
        },
      },
    },
  },
